...
/**
 * @brief Save a full Type 2 dump with a user-facing name in an SD directory.
 *
 * The name is converted to a FAT-safe filename and made unique. The directory
 * must be an absolute path relative to the SD root (for example
 * `/nfc/MyAmiibo`).
 */
 ...
 *
 * @param[in] rel_path Dump file path relative to SD root (e.g. "/nfc/x.nfc").
